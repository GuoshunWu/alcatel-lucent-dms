define ['jqgrid', 'require'], ($, require)->
  ...
  dicGrid = $('#stringSettingsGrid').jqGrid(
    ...
    colModel: [
      ...
      {name: 'context', index: 'context.name', editrules: {required: true}, width: 80, editable: true, classes: 'editable-column', align: 'left'}
      {name: 'description', index: 'description', width: 60, editable: true, classes: 'editable-column', align: 'left'}
      {name: 't', index: 't', sortable: false, width: 20, align: 'right'}
      {name: 'n', index: 'n', sortable: false, width: 20, align: 'right'}
      {name: 'i', index: 'i', sortable: false, width: 20, align: 'right'}
    ]
