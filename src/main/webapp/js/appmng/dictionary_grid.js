...
(function() {
  ...
  define(['jqgrid', 'require', 'util', 'appmng/dialogs'], function($, require, util, dialogs) {
      ...
      beforeProcessing: function(data, status, xhr) {
        ...
        return $(data.rows).each(function(index) {
          ...
          return this.cell[actIndex] = ($(['S', 'L', 'X']).map(function() {
            return "<A href=\"javascript:dctgrid=require('appmng/dictionary_grid');                      dctgrid.cellAction('" + this + "'," + rowData.id + ")\" id='" + ids[this] + "'>" + this + "</A>";
          })).get().join('');
        });
      }
    });
    ...
    return {
      ...
      cellAction: function(action, rowId) {
        var rowData;
        rowData = $('#dictionaryGridList').getRowData(rowId);
        dialogs.langSetting.data("param", {
          dictId: rowId,
          refCode: rowData.langrefcode
        });
        return dialogs.langSetting.dialog('open');
      }
    };
  });
