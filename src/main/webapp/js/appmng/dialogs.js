...
(function() {
  define(['jqueryui', 'jqgrid', 'blockui', 'jqmsgbox', 'i18n!nls/common', 'i18n!nls/appmng', 'dms-urls', 'dms-util', 'appmng/dictlistpreview_grid', 'appmng/stringsettings_grid', 'appmng/report_chart', 'appmng/dictpreviewstringsettings_grid', 'appmng/previewlangsetting_grid', 'appmng/searchtext_grid'], function($, jqgrid, blockui, msgbox, c18n, i18n, urls, util, previewgrid, stgrid, chart) {
    var addApplication, addLanguage, dictListPreview, dictPreviewLangSettings, dictPreviewStringSettings, historyDlg, importReport, langSettings, lockLabels, newAppVersion, newProductVersion, removeLanguage, searchResult, setContextTo, showSearchResult, stringSettings, stringSettingsTranslation;
      ...
      buttons: [
        {
          ...
          click: function(e) {
            ...
            postData = {
              dicts: $(this).data('param').dicts.join(','),
              languageId: $('#languageName', this).val(),
              charsetId: $('#charset', this).val(),
              code: $('#addLangCode', this).val()
            };
            $('#errorMsg', this).empty();
            ...
            return $.post(urls.app.add_dict_language, postData, function(json) {
              $(_this).unblock();
              if (json.status !== 0) {
                $.msgBox(json.message, null, {
                  title: c18n.error
    ...
    removeLanguage = $('#removeLanguageDialog').dialog({
      ...
      create: function(event, ui) {},
      open: function(e, ui) {
        return $('#removeLanguageDialog_errorMsg', this).empty();
      },
      buttons: [
        {
          text: c18n['del'],
          ...
          click: function(e) {
            ...
            postData = {
              dicts: $(this).data('param').dicts.join(','),
              code: $('#removeLanguageDialog_addLangCode', this).val()
            };
            $('#removeLanguageDialog_errorMsg', this).empty();
            if (!postData.code) {
              if (!postData.code) {
                $('#removeLanguageDialog_errorMsg', this).append($("<li>" + i18n.dialog.addlanguage.coderequired + "</li>"));
              }
              return;
            }
            $(this).block({
            ...
            return $.post(urls.app.remove_dict_language, postData, function(json) {
              $(_this).unblock();
              if (json.status !== 0) {
                $.msgBox(json.message, null, {
                  title: c18n.error
